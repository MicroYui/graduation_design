...
class one_action_environment(object):
    ...
    def update_request_arrive_array_matrix(self):
        ...
        for index in range(len(self.start_service)):
            ...
            route_vector = self.simple_route(compute_and_transmit_time[0], compute_and_transmit_time[1])
    ...
    def update_state(self, state):
        ...
        instance_vector = pre_state.reshape(self.services, self.nodes).astype(int)
    ...
    def reset(self):
        ...
        return instance_reset

    def constrains_reset(self):
        while True:
            instance_reset = torch.tensor(np.random.randint(2, size=(self.services, self.nodes))).flatten()
            state = instance_reset
