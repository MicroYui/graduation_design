...
class PSO(SkoBase):
    def __init__(self, func, n_dim=None, pop=40, max_iter=150, lb=None, ub=None, w=0.8, c1=0.5, c2=0.5,
                 constraint_eq=tuple(), constraint_ueq=tuple(), verbose=False
                 , dim=None, precision=None):
    ...
    def run(self, max_iter=None, precision=None, N=20):
        ...
        for iter_num in range(self.max_iter):
            ...
            # add ga here
            # Calculate Fit , GAnum , GAps and GAmaxi
            ga_num_max, ga_num_min = 80, 1
            ga_num = int(ga_num_max - (iter_num / self.max_iter) ** 10 * (ga_num_max - ga_num_min))

            ga_ps_min, ga_ps_max = 10, 15
            ga_ps = int(ga_ps_min + (iter_num / self.max_iter) ** 10 * (ga_ps_max - ga_ps_min))
            ga_ps += ga_ps % 2

            ga_mini, ga_maxi = 40, 80
            ga_imax = int(ga_mini + (iter_num / self.max_iter) ** 15 * (ga_maxi - ga_mini))

            # Randomly select GAnum particles
            rand_idx = np.random.choice(self.pop, size=ga_num, replace=False)
            selected_particles = self.X[rand_idx]
            for i, particle in enumerate(selected_particles):
                ga = GA(func=self.ga_func,
                        n_dim=self.n_dim,
                        size_pop=ga_ps,
                        max_iter=ga_imax,
