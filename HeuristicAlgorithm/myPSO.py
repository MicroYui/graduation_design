...
class PSO(SkoBase):
    def __init__(self, func, n_dim=None, pop=40, max_iter=150, lb=-1e5, ub=1e5, w=0.8, c1=0.5, c2=0.5,
                 constraint_eq=tuple(), constraint_ueq=tuple(), verbose=False, state=None, services=5, nodes=5
                 , dim=None):
    ...
    def update_X(self):
        ...
        for i in range(self.pop):
            self.X[i][:self.services * self.nodes] = np.round(self.X[i][:self.services * self.nodes], decimals=0)
